 ...
 */
define("assignsubmission_cloudpoodll/modalformhelper",["jquery","core/log","core/str","core/modal_factory","core/modal_events","core/fragment","core/ajax","core/yui"],(function($,log,Str,ModalFactory,ModalEvents,Fragment,Ajax,Y){var TheForm=function(selector){this.init(selector)};return TheForm.prototype.modal=null,TheForm.prototype.init=function(selector){$(selector);var dd=this;$("body").on("click",selector,(function(e){e.preventDefault(),dd.modaldata={contextid:$(this).data("contextid"),mediaurl:$(this).data("mediaurl"),mediatype:$(this).data("mediatype"),transcripturl:$(this).data("transcripturl"),lang:$(this).data("lang")},ModalFactory.create({type:ModalFactory.types.CANCEL,title:dd.formtitle,body:dd.getBody(dd.modaldata)}).then((function(modal){return dd.modal=modal,dd.modal.getRoot().on(ModalEvents.hidden,function(){dd.modal.setBody(dd.getBody(dd.modaldata))}.bind(dd)),dd.modal.show(),dd.modal}))}))},TheForm.prototype.getBody=function(modaldata){return Fragment.loadFragment("assignsubmission_cloudpoodll","mform",modaldata.contextid,modaldata)},{init:function(selector){return new TheForm(selector)}}}));
