...
class utils
{
    ...
    public static function fetch_options_recorders()
    {
        $rec_options = array(constants::REC_AUDIO => get_string("recorderaudio", constants::M_COMPONENT),
            ...
            constants::REC_WHITEBOARD => get_string("recorderwhiteboard", constants::M_COMPONENT));
        return $rec_options;
    }

    public static function fetch_options_boardsizes()
    {
        $boardsizes = array(
            '320x320' => '320x320',
            '400x300' => '400x300',
    ...
    public static function fetch_token($apiuser, $apisecret, $force = false, $debug = false)
    {
        ...
        if ($token_response) {
            if ($debug) {
                $dbg['token_response'] = $token_response;
            }

            $resp_object = json_decode($token_response);
            if ($debug) {
                if ($resp_object) {
                    $dbg['resp_object'] = $resp_object;
                }
            }
    ...
    public static function cleanup_files($trace = false)
    {
        ...
        if (!$assignids) {
            if ($trace) {
                $trace->output('No assignments to clean up, exiting');
            }
            return true;
        }
        ...
        foreach ($assignids as $assignid) {
            if ($trace) {
                $trace->output("Cleaning up assignment $assignid");
            }
